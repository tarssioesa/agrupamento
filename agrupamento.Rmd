...
Esses resultados são bastante heterogêneos entre as grandes regiões do país. Enquanto as regiões Centro-Oeste, Sudeste e Sul têm coberturas de abastecimento próximas ou acima de 90%, as regiões Norte e Nordeste apresentam valores bem mais baixos.
O desafio da universalização torna-se ainda maior quando se consideram as metas de coleta e tratamento de esgoto (90% da população até 2033).
...
- **Espectrais/graph-based**: usa autovetores de grafos de similaridade.  
- Há outros métodos que podem ser trabalhados ao longo deste ou de outro documento.
---
...
### O que é *k*-means e como usar **silhouette**
- **k-means** particiona os dados em *k* grupos minimizando a soma das distâncias quadráticas aos **centróides** (variância intra-clusters).  
...
- **Davies–Bouldin (DB)** — minimizar. Penaliza sobreposição; útil para comparar k maiores.   
...
DT::datatable(
  tab,
  rownames = FALSE,
  fillContainer = TRUE,                 # ocupa o contêiner
  options = list(
    paging = FALSE,                     # mostra TODAS as linhas (as 10)
    # ou, se preferir manter paginação:
    # pageLength = nrow(tab), lengthChange = FALSE,
    autoWidth = TRUE,                   # ajusta largura das colunas
    scrollX = TRUE,                     # rolagem horizontal se precisar
    info = FALSE, searching = FALSE     # limpa barra de info/busca (opcional)
  ),
